 ...
 */
function GridCellShadowGfxComponent(props: GridCellShadowGfxProps) {
  const {
    pathData,
    transformedPathData,
    transformPoint,
...
// Memoize to avoid expensive shadow recalculations
export const GridCellShadowGfx = memo(
  GridCellShadowGfxComponent,
  (prev, next) => {
    // Only re-render if shadow-related props change
    return (
      prev.gradientType === next.gradientType &&
      prev.lineCount === next.lineCount &&
      prev.extendDistance === next.extendDistance &&
      prev.colorStart === next.colorStart &&
      prev.colorEnd === next.colorEnd &&
      prev.blur === next.blur &&
      prev.opacity === next.opacity &&
      prev.type === next.type &&
      prev.transformedPathData === next.transformedPathData
      // pathData and transformPoint are stable from parent
    );
  },
);
